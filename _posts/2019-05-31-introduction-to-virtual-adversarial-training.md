...
There are several regularization techniques which prevent overfitting and help the model generalize better for unseen examples. Regularization helps the model parameters less dependent on the training data. The two most commonly used regularization techniques are Dropout and L1/L2 regularization.  

In L1/L2 regularization, we add a loss term which tries to reduce the L1 norm or the L2 norm of the weights matrix. Small values of the weights would result in simpler models which are less prone to overfitting.  
...
Local distributional smoothness (LDS) can be defined as the smoothness of the output distribution of the mode with respect to the input.  We do not want the model to be sensitive to small perturbations in the inputs or we can say there should not be large changes in the model output with respect to small changes in the model input.  

In LDS regularization, smoothness of the model distribution is rewarded. It is also invariant of the parameters on the network and only depends on the model outputs.  Having a smooth model distribution should help the model generalize better because the model would give similar outputs for unseen examples which are close to examples in the training set.  Several studies show that making the model robust against small random perturbations is effective for regularization. 

An easy way for LSD regularization is by generating artificial data-points by applying small random perturbations on real data points and encouraging the model to have similar outputs for the real and perturbated data points. Domain knowledge can also be used to make better perturbations. For example, if the inputs are images, various image augmentation techniques such as flipping, rotating, transforming the color can be used.





![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction0.png?style=centerme)
...
Virtual adversarial training is an effective technique for local distribution smoothness. Pairs of data points are taken for which are very close in the input space but are very far in the model output space. Then the model is trained for making their outputs close to each other. To do that, a given input is taken and perturbation is found for which the model gives very different output. Then the model is penalized for sensitivity with the perturbation. 





![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction1.png?style=centerme)
*Step 1 : Generate the adversial image*
{: style="font-size: 80%; text-align: center;"}
...
![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction2.png?style=centerme)
*Step 2: Minimize the KL devergence*
{: style="font-size: 80%; text-align: center;"}
...
2. Transform _x_ by adding a small perturbation _r_, hence transformed the data point will be _T(x) = x + r_ 
...
![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction3.png?style=centerme)
...
![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction4.png?style=centerme)
...
2. Rather than discriminating between fake and real, the KL divergence of the model output is used. While training the model ( which is analogous to training the discriminator) we minimize the KL divergence. 
...
In virtual adversarial training, no labels label information is used and the perturbation is generated using just the model outputs. The perturbation is generated such that output of the perturbated input is different from the model output of the original input ( as opposed to the ground truth label). 
...
Define the ``model_input`` , the logits ``model_input`` by applying the input to the netwwork and the probability scores `p` by applying softmax activation on the logits. 
...
Finally, normalize the norm adversarial perturbation. We set the norm of `r_vadv` to a small value which is the distance to which we want to go along the adversarial direction. 
...
Now let's use some synthetic dataset to train and test the model. This dataset is two dimensional and has two classes. Class 1 data-points lie in the outer ring and the class 2 data-points lie in the inner ring. We are using only 8 data-points per class for training, and 1000 data-points for testing. 





![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction5.png?style=centerme)
...
![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction6.png?style=centerme)
*Model decision boundary with virtual adversarial training*
{: style="font-size: 80%; text-align: center;"}

For this example dataset, it is pretty evident that the model with virtual adversarial training has a better generalization and its decision boundary lies in the bounds of the test data as well. 





![]({{ site.baseurl }}/assets/images/posts/vat/An-Introduction7.png?style=centerme)
...
**VAT for semi-supervised learning:** Virtual adversarial training has shown good results in semi-supervised learning. Here we have a large number of unlabeled data-points and small labeled data points. Applying the `vat_loss` on the unlabeled set and the supervised loss on the labeled set gives a good boost. The authors show the superiority of the method over several other semi-supervised learning methods. You can read more in the paper [here](https://arxiv.org/abs/1704.03976).

**Virtual adversarial ladder networks**: [Ladder networks](https://arxiv.org/abs/1507.02672) have shown promising results for semi-supervised classification. There, at each input layer, random noise is added and a decoder is trained to reconstruct the outputs at each layer without noise. In virtual adversarial ladder networks, rather than random noise, adversarial noise is used. You can read more in the paper [here](https://arxiv.org/abs/1711.07476).

**Unsupervised clustering using self-augmented training**: Here the goal is to cluster the data-points in a fixed number of clusters without using any labeled samples  [Regularized Information Maximization](https://papers.nips.cc/paper/4154-discriminative-clustering-by-regularized-information-maximization) is a technique for unsupervised clustering. Here the mutual information between the input and the model output is maximized. [IMSAT](https://arxiv.org/abs/1702.08720) is extended the approach by adding virtual adversarial training. Along with the mutual information loss, the authors apply the `vat_loss`. They show great improvements after adding virtual adversarial training. You can read more in the [paper](https://arxiv.org/abs/1702.08720) and my earlier [blog post.](https://divamgupta.com/unsupervised-learning/2019/03/08/an-overview-of-deep-learning-based-clustering-techniques.html) 
